...
func Favorite(videoIdInt64 int64, userIdInt64 int64, actionType int32) (err error) {
	...
	if actionType == 1 {
		...
		dao2.RedisClient.Incr(context.Background(), util.AuthorBeLikedNum+strconv.FormatInt(userIdInt64, 10))
		...
		//业务逻辑
		result := dao2.DB.Model(&domain.Video{}).
		...
		//点赞了现在取消
		dao2.RedisClient.SRem(context.Background(), util.VideoFavoriteKeyPrefix+strconv.FormatInt(userIdInt64, 10), videoIdInt64)

		//2.total_favorite(当前用户获赞数量）++  使用redis做，TODO 数据库中可以不存这个字段
		//Incr 方法用于递增 Redis 中的整数值键。如果键不存在，它会将键的值初始化为 0，然后再执行增加操作
		dao2.RedisClient.Decr(context.Background(), util.AuthorBeLikedNum+strconv.FormatInt(userIdInt64, 10))
...
func FavoriteList(userIdInt64 int64) (videoList []domain.Video, err error) {
	...
	userFavoriteVideosIdStrArr, err := dao2.RedisClient.
