...
func Chat(c *gin.Context) {
	...
	if err1 != nil || err2 != nil {
		c.JSON(http.StatusOK, domain2.Response{StatusCode: 1, StatusMsg: "toUserId解析失败"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusOK, domain2.Response{StatusCode: 1, StatusMsg: "获取聊天记录失败"})
		return
	}
	...
	c.JSON(http.StatusOK, domain2.ChatResponse{
...
func ChatAction(c *gin.Context) {
	...
	if err1 != nil || err2 != nil {
		c.JSON(http.StatusOK, domain2.Response{StatusCode: 1, StatusMsg: "userId或actiontype解析失败"})
		return
	}
	if content == "" || actionType != 1 {
		c.JSON(http.StatusOK, domain2.Response{StatusCode: 1, StatusMsg: "参数错误"})
		return
	}
	message, err := service.AddMessage(fromUserId, toUserId, content)
	if err != nil {
		log.Println(message, err)
		c.JSON(http.StatusOK, domain2.Response{StatusCode: 1, StatusMsg: message + err.Error()})
		return
	}
	...
	c.JSON(http.StatusOK, domain2.Response{StatusCode: 0, StatusMsg: message})
}
